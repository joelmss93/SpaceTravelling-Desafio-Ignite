...
export default function Post({
  ...
  const estimatedTimeToRead = post.data.content.reduce((acc, content) => {
    ...
    const totalHeading = content.heading.split(' ').length;
    const totalOnBody = RichText.asText(content.body).split(' ').length;

    const total = totalOnBody + totalHeading + acc;
    const min = Math.ceil(total / 200) + 1;
    return min;
  }, 0);
  ...
  const formatDate = (date: Date): string => {
    console.log(date);
    const formattedDate = format(new Date(date), "dd MMM yyyy, 'às' HH:mm", {
      locale: ptBR,
    });
        ...
        <div className={styles.headerPost}>
          ...
          {post.first_publication_date !== post.last_publication_date && (
            <div className={styles.editPostContent}>
              <span>
                {/* * editado em {formatDate(new Date(post.last_publication_date))} */}
                {/* {format(
